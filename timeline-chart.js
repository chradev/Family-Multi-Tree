(function (global, factory) {
    ...
    var TimelineChart = function () {
        function TimelineChart(element, data, opts) {
            ...
            var groupWidth = options.hideGroupLabels ? 0 : 120;
            if (options.groupLabels)
              groupWidth = options.hideGroupLabels ? 0 : options.groupLabels;
            ...
            svg.append('rect')
                      ...
                      .attr('x', groupWidth)
                      .attr('y', 0)
                      .attr('height', height).attr('width', width - groupWidth);
            ...
            if (options.timeSelected) {
                self.sel = svg.append('line')
                              .attr('clip-path', 'url(#chart-content)')
                              .attr('class', 'vertical-marker sel')
                              .attr("y1", 0)
                              .attr("y2", height);
            }
            ...
            if (!options.hideGroupLabels) {
               ...
               var lineSection = svg.append('line')
                                    .attr('x1', groupWidth)
                                    .attr('x2', groupWidth)
                                    .attr('y1', 0)
                                    .attr('y2', height)
            ...
            var intervals = groupIntervalItems.append('rect')
            ...
            var intervalTexts = groupIntervalItems.append('text')
                                                  .text(function (d) { return d.label; })
                                                  .attr('fill', 'white')
                                                  .attr('class', withCustom('interval-text'))
                                                  .attr('y', groupHeight / 2 + 5).attr('x', function (d) {
                                                      return x(d.from);
                                                  });

            var groupDotItems = svg.selectAll('.group-dot-item')
                                  .data(data)
                                  .enter()
                                  .append('g')
                                  .attr('clip-path', 'url(#chart-content)')
                                  ...
                                  .selectAll('.dot')
                                  .data(function (d) { 
                                      return d.data.filter(function (_) { return _.type === TimelineChart.TYPE.POINT; });
                                   })
                                   .enter();

            var dotTexts = groupDotItems.append('text')
                                        .text(function (d) { return d.label; })
                                        .attr('fill', 'black')
                                        .attr('class', withCustom('dot-text'))
                                        .attr('y', groupHeight / 3 + 1)
                                        .attr('x', function (d) {return x(d.at)});

            //tooltip
            var tooltip = d3.select("body").append("div").attr("class", "tooltip tt-timeline");

            var dots = groupDotItems.append('circle')
                          .attr('class', withCustom('dot'))
                          .attr('cx', function (d) { return x(d.at); })
                          .attr('cy', groupHeight / 2)
                          .attr('r', function (d) { return groupHeight / 4; })
                          ...
                          .on('mousemove', function (d) {
                              tooltip.html(options.tooltip(d))
            ...
            function updateNowMarker() {
                ...
                var selX = 0;
                if (options.timeSelected && options.timeSelected()) {
                      dateSelected = new Date(options.timeSelected());
                      selX = x(dateSelected);
                }
                self.sel.attr('x1', selX).attr('x2', selX);
            }
            ...
            function zoomed() {
                ...
                svg.selectAll('.dot-text').attr('x', function (d) {
                    var positionData = getTextPositionData.call(this, d);
                    if (positionData.upToPosition - groupWidth - 10 < positionData.textWidth) {
                        return positionData.upToPosition;
                    } else if (positionData.xPosition < groupWidth && positionData.upToPosition > groupWidth) {
